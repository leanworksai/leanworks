...
def get_client_info(user_id: str) -> tuple:
    """
    Get client domain from user_id email and query available tools from integrations table.
    ...
    Returns:
        ...
        - available_tools: List of tool names from integrations table where connected = true
    """
    try:
        ...
        # Query integrations table for available tools
        available_tools = _get_available_tools_from_integrations(domain)
        
        logger.info(f"Retrieved domain for user {user_id}: {domain}, available tools: {available_tools}")
        return domain, available_tools
...
def _get_available_tools_from_integrations(domain: str) -> List[str]:
    """
    Query the integrations table from postgres to get available tools.
    
    Args:
        domain: The client domain (e.g., 'leanworks.ai')
        
    Returns:
        List of tool names (e.g., ['outlook', 'gitlab']) that are connected
    """
    try:
        from leanworks.agent.tools.postgres import PostgresTool
        
        # Create a simple wrapper for PostgresTool
        class PostgresClientWrapper:
            def __init__(self, domain):
                self.domain = domain
        
        postgres_client_wrapper = PostgresClientWrapper(domain)
        
        # Initialize PostgresTool to query the integrations table
        postgres_tool = PostgresTool(postgres_client_wrapper)
        
        # Query integrations table for connected integrations
        # The table structure may vary, so we'll try common column names
        query = """
            SELECT 
                COALESCE(integration_name, name, integration_id, id) as tool_name,
                connected
            FROM integrations
            WHERE connected = true
        """
        
        results = postgres_tool.query_postgres(sql=query)
        
        # Handle error case
        if isinstance(results, dict) and "error" in results:
            logger.warning(f"Failed to query integrations table: {results.get('error')}")
            return []
        ...
        for row in results:
            ...
            if tool_name:
                ...
                # Skip internal tools (they're always available)
                if tool_name not in ['search', 'postgres', 'duckdb']:
                    tools.append(tool_name)
        
        logger.info(f"Found {len(tools)} available tools from integrations table: {tools}")
        return tools
        
    except Exception as e:
        logger.warning(f"Error querying integrations table for available tools: {str(e)}")
        traceback.print_exc()
        return []